...
export default function PerspectiveView({ floors, zones, cameras, pois, signageItems, transitions, config }: Props) {
  ...
  useEffect(() => {
    ...
    setBuilding(true)
    sceneRef.current.buildFromData({ floors, zones, cameras, pois, signageItems, transitions, floorStack: config.floorStack, zoneHeights: config.zoneHeights, mode: 'perspective', config })
    setBuilding(false)
