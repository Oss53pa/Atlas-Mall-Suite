      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.04]">
        <div className="flex items-center gap-2.5">
          <div className="relative">
            <div className="w-7 h-7 rounded-lg bg-atlas-500/10 border border-atlas-500/20 flex items-center justify-center">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && (
          <div className="text-center mt-12 animate-fade-in">
            <div className="w-14 h-14 rounded-2xl bg-atlas-500/10 border border-atlas-500/15 flex items-center justify-center mx-auto mb-4">
        ...
        {messages.map(msg => (
          <div key={msg.id} className={`flex gap-2.5 ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-slide-up`}>
            {msg.role !== 'user' && (
              <div className="w-6 h-6 rounded-md bg-atlas-500/10 flex items-center justify-center flex-none mt-0.5">
                <Bot className="w-3.5 h-3.5 text-atlas-400" />
              </div>
            )}
        ...
        {isProcessing && (
          <div className="flex gap-2.5 justify-start animate-fade-in">
            <div className="w-6 h-6 rounded-md bg-atlas-500/10 flex items-center justify-center flex-none">
              <Bot className="w-3.5 h-3.5 text-atlas-400" />
            </div>
      ...
      <form onSubmit={handleSubmit} className="p-3 border-t border-white/[0.04]">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-2.5 rounded-xl bg-atlas-600 text-white hover:bg-atlas-500 disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-150 shadow-glow-purple hover:shadow-glow-purple"
          >
