...
export async function exportToPDF(
  ...
  doc.text(`Document de planification — ${new Date().toLocaleDateString('fr-CI')} — Atlas BIM Vol.1`, 14, 30)
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(`Généré par Atlas BIM — Proph3t Engine — Page ${i}/${pageCount}`, 14, 205)
  }
...
export async function exportToPPTX(
  ...
  s1.addText(`${new Date().toLocaleDateString('fr-CI')} — Atlas BIM Vol.1`, { x: 0.5, y: 2, w: 5, fontSize: 11, color: '808080' })
