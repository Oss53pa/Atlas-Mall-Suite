...
JSON STRICT seulement, pas de texte autour.`,
  },
  {
    ...
    description: 'Classifier un label DXF dans la taxonomie 31 types Atlas BIM',
...
Réponse JSON : { "type": "...", "confidence": 0..1, "reason": "..." }`,
  },
  {
    ...
    defaultPromptTemplate: `Tu es PROPH3T, orchestrateur d'Atlas BIM.
Contexte : {CONTEXT}
Question : {QUESTION}
    ...
    if (status && !status.installed && status.fallbackUsed) {
      ...
      try {
        ...
        const response = await proph3tQuery(input.prompt, {
          orgName: 'Atlas Studio', projectName: 'The Mall',
...
export function generateInstallScript(): string {
  const lines = [
    '#!/bin/bash',
    '# Atlas BIM — installation modèles Ollama embarqués',
