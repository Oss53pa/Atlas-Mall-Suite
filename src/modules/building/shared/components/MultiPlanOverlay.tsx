...
export function MultiPlanOverlay({ floorId, className = '', onConsolidated }: MultiPlanOverlayProps) {
  ...
  const consolidatePng = useCallback(async () => {
    ...
    ctx.fillText(`Atlas BIM — ${new Date().toLocaleDateString('fr-FR')} — ${visibleLayers.length} niveau(x) consolidés`, 12, maxH - 14)
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 ${maxW} ${maxH}" width="${maxW}" height="${maxH}">
  ...
  ${layersXml.join('\n  ')}
  <text x="12" y="${maxH - 14}" font-family="sans-serif" font-size="14" fill="rgba(0,0,0,0.5)">Atlas BIM — ${new Date().toLocaleDateString('fr-FR')} — ${visibleLayers.length} niveau(x) consolidés</text>
</svg>`
  }, [layers])
