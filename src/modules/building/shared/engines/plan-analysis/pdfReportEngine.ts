...
export async function generateSignagePdfReport(input: PdfReportInput): Promise<Blob> {
  ...
  for (let i = 1; i <= total; i++) {
    ...
    doc.text('PROPH3T · Atlas BIM', pageW / 2, pageH - 6, { align: 'center' })
  }
...
function renderCoverPage(doc: jsPDF, input: PdfReportInput, pageW: number, pageH: number) {
  ...
  doc.setFontSize(7)
  doc.setTextColor('#64748b')
  doc.text('Rapport généré automatiquement par PROPH3T · Atlas BIM · 2026', pageW / 2, pageH - 4, { align: 'center' })
}
...
function renderMethodology(doc: jsPDF, y: number, margin: number, width: number): number {
  ...
  const sections = [
    ...
    },
    {
      ...
      text: 'Score = 40% × couverture nœuds de décision + 30% × continuité de guidage ERP + 20% × lisibilité panneaux (distance + angle) + 10% × accessibilité PMR. Chaque composante est calculée indépendamment et pondérée selon la spécification Atlas BIM.',
    },
    {
