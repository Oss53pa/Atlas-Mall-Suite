      ...
      <div className="relative w-full max-w-md px-6">
        ...
        <div className="rounded-2xl border border-white/[0.08] p-8" style={{ background: '#0e1629' }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div><label className="text-[11px] text-gray-500 mb-1 block">Nom complet</label>
              ...
              <input value={form.fullName} onChange={e => setForm({...form, fullName: e.target.value})} placeholder="Cheick Sanankoua"
                className="w-full bg-[#141e2e] text-white text-sm rounded-xl pl-9 pr-3 py-2.5 border border-white/[0.08] outline-none focus:border-indigo-500/50 placeholder:text-gray-600" /></div></div>
          ...
          <div className="mt-5 text-center">
            <Link to="/login" className="text-sm text-indigo-400 hover:text-indigo-300 transition-colors">Déjà un compte ? Se connecter</Link>
          </div>
        </div>
      </div>
    </div>
