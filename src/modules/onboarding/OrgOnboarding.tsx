    ...
    <div className="min-h-screen flex items-center justify-center" style={{ background: '#060a13' }}>
      <div className="relative w-full max-w-lg px-6">
        <div className="flex flex-col items-center mb-8">
          ...
          <div className="flex items-center gap-2 mb-2">{[1,2,3,4].map(i => <div key={i} className={`w-8 h-1 rounded-full ${i<=2?'bg-indigo-500':'bg-gray-700'}`}/>)}</div>
        ...
        <div className="rounded-2xl border border-white/[0.08] p-8" style={{ background: '#0e1629' }}>
          <div className="flex items-center gap-2 mb-6"><Building2 size={18} className="text-indigo-400" /><h2 className="text-lg font-semibold text-white">Informations de l'organisation</h2></div>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div><label className="text-[11px] text-gray-500 mb-1 block">Raison sociale *</label>
              <input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="New Heaven SA"
                className="w-full bg-[#141e2e] text-white text-sm rounded-xl px-3 py-2.5 border border-white/[0.08] outline-none focus:border-indigo-500/50 placeholder:text-gray-600"/></div>
            ...
            <div className="flex justify-between pt-4">
              ...
              <button type="submit" disabled={loading} className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-sm font-medium">
