    ...
    <div className="min-h-screen flex items-center justify-center" style={{ background: '#1a1d23' }}>
      <div className="relative w-full max-w-lg px-6">
        <div className="flex flex-col items-center mb-8">
          ...
          <div className="flex items-center gap-2 mb-2">{[1,2,3,4].map(i=><div key={i} className="w-8 h-1 rounded-full bg-atlas-500"/>)}</div>
        ...
        <div className="rounded-2xl border border-white/[0.08] p-8" style={{ background: '#262a31' }}>
          ...
          <button onClick={()=>setInvites([...invites,{email:'',role:'viewer'}])} className="flex items-center gap-1.5 text-sm text-atlas-400 hover:text-atlas-300 mb-6"><Plus size={14}/>Ajouter</button>
          <div className="flex justify-between pt-4 border-t border-white/[0.04]">
            ...
            <button onClick={handleSubmit} disabled={loading} className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-atlas-500 hover:bg-atlas-500 disabled:opacity-50 text-white text-sm font-medium">
